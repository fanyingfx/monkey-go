...
func (vm *VM) buildHash(startIdex, endIndex int) (object.Object, error) {
	hashedPairs := make(map[object.HashKey]object.HashPair)
	for i := startIdex; i < endIndex; i += 2 {
		key := vm.stack[i]
		value := vm.stack[i+1]
...
func (vm *VM) pushClosure(constIndex, numFree int) error {
	...
	// copy free values ,then clear stack
	for i := 0; i < numFree; i++ {
...
func (vm *VM) callClosure(cl *object.Closure, numArgs int) error {
	...
	frame := NewFrame(cl, vm.sp-numArgs) // reset basePointer to vm.sp-numArgs
	vm.pushFrame(frame)
