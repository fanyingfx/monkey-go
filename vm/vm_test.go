...
func testExpectedObject(t *testing.T, expected any, actual object.Object) {
	...
	case []int:
		...
		for i, exexpectedElem := range expected {
			err := testIntegerObject(int64(exexpectedElem), array.Elements[i])
			if err != nil {
				t.Errorf("testIntegerobject failed: %s", err)
			}
		}
	...
	default:
		t.Errorf("type of object not handled. got=%T", actual)
...
func testStringObject(expected string, actual object.Object) error {
	...
	if str.Value != expected {
		return fmt.Errorf("object has wrong value. got=%s, want=%s", str.Value, expected)
	}
	return nil
}
