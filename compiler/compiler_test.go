...
func TestCompilerSocpes(t *testing.T) {
	compiler := New()
	...
	if compiler.scopeIndex != 1 {
		t.Errorf("socpeIndex wrong. got=%d, want=%d", compiler.scopeIndex, 1)
	}
...
func testConstants(t *testing.T, expected []any, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testStringObjectg failed: %s", i, err)
			}
